...
`OSS/Grub` пока является абстрактной границей для будущих политик и не выбирает
формат исходной конфигурации или команду генерации `grub.cfg`. Финальный
`Grub::apply()` получает валидированное значение текущей политики, сериализует
операции всех GRUB-политик одним mutex и передает значение в чисто виртуальный
hook `applyGrub()`. Конкретный наследник обязан реализовать persistent-
изменение, безопасную runtime/boot-активацию, если она физически возможна, и
проверку постусловий. Сам класс `Grub` не зарегистрирован в `PolicyMap`, а
конкретных GRUB-политик пока нет.
