...
`has_more` и `next_cursor`. Cursor версии 1 привязан к `boot_id` и хранит для
каждого файла его относительный путь, `st_dev`, `st_ino` и byte offset, поэтому
добавление строки в одном файле не сдвигает позицию другого. Новый файл читается
с начала. Исчезновение/смена inode или усечение ниже сохранённого offset
возвращает `reload_required=true`; клиент должен очистить текущую модель и
