...
- поддерживает карту модулей, подмодулей и политик через `init_cafMap()`;
...
- Низкоуровневые общие утилиты находятся в `fic-common/fic-core`; старые заголовки `fic/src/utils/*.h` сохранены как совместимые обертки на время поэтапной миграции.
...
- После изменения конфигурации демон должен перечитывать `cafMap`, чтобы последующие операции работали с актуальным состоянием.
