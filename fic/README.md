...
Текущее значение из `/proc/sys` проверяется и несовпадение журналируется, но
эта версия не запускает `sysctl -w` и не изменяет работающий kernel runtime.
Такое применение требует отдельного контракта проверяемого системного вызова и
отката; текущая реализация исправляет только постоянную конфигурацию.
