...
выполняется через `VerifiedProcessExecutor`, поэтому администратор должен
предварительно сохранить эталонный hash выбранного файла, например:
...
`VerifiedProcessExecutor`. Перед применением SSH-политик администратор должен
сохранить hash фактически выбранных исполняемых файлов, например:
...
Единый реестр устраняет копии списков путей в политиках, но намеренно не
авторизует новый hash во время обычного runtime: отсутствие эталона или его
несовпадение по-прежнему приводит к fail-closed отказу. Автоматическое
первичное заполнение эталонов должно выполняться отдельным доверенным
package-transaction механизмом, а не самим resolver.
