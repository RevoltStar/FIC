...
Debian/Ubuntu-пакет объявляет `dpkg` file triggers для `/usr/bin`, `/usr/sbin`,
`/bin` и `/sbin`; ALT-пакет устанавливает нативный для ALT исполняемый
`/usr/lib/rpm/fic-trust-sync.filetrigger`, который получает от RPM список
измененных транзакцией файлов и реагирует на те же каталоги. Первичная
синхронизация выполняется до включения сервисов. Ошибка пакетной проверки
останавливает hook и не меняет ни один эталон.
