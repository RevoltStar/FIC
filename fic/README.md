...
`/bin` и `/sbin`; ALT-пакет использует RPM transaction file trigger для тех же
каталогов. Первичная синхронизация выполняется до включения сервисов. Ошибка
пакетной проверки останавливает hook и не меняет ни один эталон.
