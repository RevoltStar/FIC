...
непрозрачный `next_cursor`, пока `has_more` остается истинным. Daemon хранит в
cursor отдельные inode- и byte-позиции каждого файла, поэтому порядок обхода и
append в соседний файл не меняют уже прочитанные позиции. Новый файл начинается
с offset 0. Смена inode, исчезновение или усечение известного файла требуют
