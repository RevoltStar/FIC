...
и проверка хэшей команд. Старые заголовки в `fic/src/utils` временно оставлены
как совместимый forwarding-слой для постепенной миграции.
...
модулей остаются в `fic/src/modules`, а старые заголовки в `fic/src/core` и
`fic/src/utils/PolicyTypeValue.h` временно перенаправляют на новую библиотеку.
