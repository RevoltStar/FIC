...
Security audit trail обоих daemon сериализуется общей реализацией `fic-core`
как JSON Lines. IPC event содержит authoritative `timestamp`, `component`,
typed `peer`, `command`, компонентный whitelist `request` и `result`; request
не копируется целиком. Ограничения строк и полной записи применяются до
финального `json::dump()`, поэтому каждая физическая строка остаётся валидным
самостоятельным JSON-объектом даже при усечении.
...
flowchart TD
    ...
    logViewer --> records[log_records pages<br/>opaque per-file cursor / limit до 500]
```
...
Логи загружаются страницами не более 500 записей и 768 КиБ; GUI передаёт
непрозрачный token `next_cursor`, пока `has_more` остается истинным. Daemon
держит в ограниченном cursor store отдельные inode- и byte-позиции каждого
файла, а wire token имеет фиксированный размер. Поэтому порядок обхода и append
в соседний файл не меняют уже прочитанные позиции и не увеличивают следующий
IPC request. Новый файл начинается с offset 0; offset продвигается только после
полностью завершённой `\n` записи. Смена inode, исчезновение или усечение
известного файла, а также утрата token после рестарта или eviction требуют
полной перезагрузки модели через `reload_required`. IPC-команды `boot_id` и
