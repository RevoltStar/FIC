# fic-gui

`fic-gui` - графический клиент FIC для просмотра и изменения настроек политик. Компонент предоставляет пользовательский интерфейс, но не владеет конфигурационными файлами напрямую.

## Назначение

GUI предназначен для удобного изменения значений политик и их состояния. В новой архитектуре `fic-gui` не пишет в `/opt/fic/config` самостоятельно. При нажатии применения изменений он отправляет команды демону `fic` через Unix-сокет.

Это сохраняет единый источник изменений: daemon API. Благодаря этому CLI, GUI и периодическая проверка работают через одну модель состояния.

## Основные возможности

`fic-gui` предоставляет:

- графический просмотр модулей, подмодулей и политик;
- редактирование значений политик через специализированные редакторы;
- включение и отключение политик;
- отправку изменений демону через команды `set_policy_value`, `enable_policy`, `disable_policy`;
- просмотр логов и фильтрацию записей;
- отображение информации об устройствах из базы FIC.

Верхнеуровневые вкладки создаются из `module_list`: обычные модули используют
`StandardModulePage`, `DC` с `view=device` — `DeviceModulePage`, а `AUDIT` с
`view=audit` — `AuditModulePage`. Неизвестный `view` отклоняется. Все три
страницы переиспользуют `PolicyEditorWidget` для стандартных политик.

## Взаимодействие с демоном

GUI использует общий IPC-клиент:

```text
fic-common/fic-ipc/include/fic/ipc/FicIpcClient.h
```

По умолчанию подключение выполняется к:

```text
/run/fic/fic.sock
```

Для разработки путь можно переопределить:

```bash
FIC_SOCKET_PATH=/tmp/fic.sock fic-gui
```

Для device API используется отдельная переменная `FIC_DEVICE_SOCKET_PATH`.
Она не подменяется значением `FIC_SOCKET_PATH`, чтобы тестовый policy daemon
нельзя было случайно принять за привилегированный device endpoint.

Версию и происхождение сборки можно проверить без запуска графической сессии:

```bash
fic-gui --version
fic-gui --build-info
```

## Обновление дерева устройств

При первой загрузке GUI получает дерево и запоминает значение
`device_tree_revision` от `fic-dick --daemon`. Затем GUI проверяет ревизию
каждые пять секунд. Если значение не изменилось, существующее дерево не
перестраивается. При новой ревизии GUI перечитывает дерево, сохраняя раскрытые
узлы, выбранное устройство и положение прокрутки.

Проверка и чтение выполняются только через device API; GUI не обращается к
`/opt/fic/db/devices.db` напрямую и не зависит от SQLite WAL-файлов.

## Сохранение и применение изменений

Кнопка «Сохранить» проходит по списку изменённых политик и для каждой политики
выполняет две операции:

1. Отправляет новое значение:

```json
{"api_version":1,"command":"set_policy_value","module":"<module>","policy":"<policy>","value":"<value>"}
```

2. Отправляет новое состояние политики:

```json
{"api_version":1,"command":"enable_policy","module":"<module>","policy":"<policy>"}
```

или:

```json
{"api_version":1,"command":"disable_policy","module":"<module>","policy":"<policy>"}
```

Если демон возвращает ошибку, GUI показывает пользователю текст ошибки и не
считает сохранение успешным. `apply_module` при этом не вызывается.

Кнопка «Сохранить и применить» сначала выполняет то же сохранение и только после
его полного успеха вызывает `apply_module`. Валидный ответ этой операции считается
завершённым независимо от `ok`: при `ok=false` GUI показывает warning, сохраняя
`message`, `summary`, `results` и `diagnostics`. IPC/service errors показываются
отдельно. GUI показывает сводку ответа демона,
а в раскрываемом поле Details — результат и диагностические записи каждого
вызова `Policy::apply()`. Диагностика учитывает настроенный уровень логирования;
достижение лимита ответа отмечается строкой `... diagnostics truncated`. Если
применение завершилось ошибкой, уже сохранённые значения и ENABLE/DISABLE не
откатываются.

## Что GUI не должен делать

`fic-gui` не должен:

- напрямую писать в `/opt/fic/config`;
- самостоятельно применять политики к ОС;
- обходить daemon API для включения, отключения или изменения значений политик;
- требовать прав на запись к системным конфигам FIC, если эту работу может выполнить демон.

## Runtime-зависимости

Для полноценной работы требуется запущенный демон `fic`.

Проверить демон можно через CLI:

```bash
fic-cli status
```

Если используется нестандартный сокет:

```bash
FIC_SOCKET_PATH=/tmp/fic.sock fic-cli status
```

## Сборка

Из корня проекта:

```bash
cmake -S . -B build-check -DFIC_TARGET_PLATFORM=ubuntu-24.04
cmake --build build-check --target fic-gui
```

Отдельная сборка компонента:

```bash
cmake -S fic-gui -B build-fic-gui
cmake --build build-fic-gui
```

## Зависимости

Компонент использует:

- C++17;
- Qt Widgets;
- SQLite3;
- OpenSSL;
- nlohmann/json;
- общие классы модулей, политик и утилит FIC.

## Упаковка

В deb/rpm packaging графический компонент устанавливается как:

```text
/opt/fic/bin/fic-gui
```

При bundle-сборке Qt реальный бинарный файл устанавливается как:

```text
/opt/fic/bin/fic-gui.real
```

а `/opt/fic/bin/fic-gui` является launcher-скриптом, который выставляет Qt-переменные окружения и запускает реальный бинарник.

Qt остаётся динамически подключаемой third-party библиотекой и не становится
частью лицензии FIC SUL-1.0. По умолчанию launcher использует минимальный runtime
из `/opt/fic/qt`. Совместимую пользовательскую сборку Qt можно подставить без
изменения FIC:

```bash
FIC_QT_ROOT=/path/to/custom/qt /opt/fic/bin/fic-gui
```

В этом каталоге ожидаются `lib/` и `plugins/`; launcher использует их вместо
bundled путей в `LD_LIBRARY_PATH`, `QT_PLUGIN_PATH` и
`QT_QPA_PLATFORM_PLUGIN_PATH`. `/opt/fic/bin/fic-gui.real` также можно запускать
напрямую с системной Qt или с самостоятельно выставленными loader/plugin paths.
Расположенный рядом `qt.conf` обеспечивает bundled layout по умолчанию, но не
запрещает замену библиотек.

Команда `fic-gui --license-info` без запуска Qt GUI прямо сообщает, что FIC
остаётся под SUL-1.0, а применимые bundled Qt components распространяются FIC
по GNU LGPL version 3 там, где доступна эта license option. Вывод также
указывает на alternative Qt terms, лицензии embedded third-party components и
полный комплект notices в `/usr/share/doc/fic-gui/`. Внутренняя packaging-проверка
`--gui-smoke-test` создаёт `QApplication` и `QWidget`, загружает JPEG через Qt
plugin и кратко выполняет event loop; она не создаёт `MainWindow` и не
обращается к daemon.

Состав bundle, package provenance, package-level license summary и хеши файлов находятся
в `/usr/share/doc/fic-gui/third-party-components.json`; сопровождающие тексты —
в `/usr/share/doc/fic-gui/licenses/`. Полный release/compliance flow описан в
[`docs/third-party-licensing.md`](../docs/third-party-licensing.md).

## Связанные файлы

- `fic-gui/src/app/main.cpp` - точка входа приложения;
- `fic-gui/src/app/MainWindow.cpp` - загрузка дескрипторов и создание верхнеуровневых вкладок;
- `fic-gui/src/features/policies` - policy models, IPC orchestration, pages и reusable editor;
- `fic-gui/src/features/devices` - device service, widgets и module page;
- `fic-gui/src/features/logs` - log models, service и viewer;
- `fic-gui/src/features/policies/widgets/PolicyEditorWidget.*` - общий редактор политик;
- `fic-gui/src/shared/i18n` - Qt-обертка локализации;
- `fic-common/fic-ipc/include/fic/ipc/FicIpcClient.h` - IPC-клиент для связи с демоном.

## Типовой сценарий разработки

1. Запустить демон на тестовом сокете:

```bash
fic --socket /tmp/fic.sock --interval 60
```

2. Запустить GUI с тем же сокетом:

```bash
FIC_SOCKET_PATH=/tmp/fic.sock fic-gui
```

3. Изменить политику в интерфейсе.

4. Проверить состояние через CLI:

```bash
FIC_SOCKET_PATH=/tmp/fic.sock fic-cli policy list all
```

## Важные правила разработки

- Новые действия GUI, которые меняют конфигурацию, должны добавляться через IPC-команды демона.
- GUI может читать данные для отображения, но запись конфигурации должна идти через `fic`.
- Ошибки IPC должны показываться пользователю достаточно подробно, чтобы было понятно, не запущен демон, недоступен сокет или отклонена команда.
- Если добавляется новая daemon-команда, стоит добавить аналогичную диагностику в `fic-cli`, чтобы сценарий можно было проверить без GUI.
