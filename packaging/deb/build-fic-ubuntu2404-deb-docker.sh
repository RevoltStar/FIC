...
IMAGE_NAME="${IMAGE_NAME:-fic-deb-builder:ubuntu2404}"
DOCKERFILE_PATH="${DOCKERFILE_PATH:-$ROOT_DIR/packaging/deb/Dockerfile.ubuntu2404}"
...
"$CONTAINER_CMD" run --rm \
    ...
    -e BUILD_ROOT="${BUILD_ROOT:-/tmp/fic-build-ubuntu2404}" \
    -e DIST_DIR="${DIST_DIR:-/workspace/dist}" \
    -v "$ROOT_DIR:/workspace" \
    -w /workspace/packaging/deb \
    "$IMAGE_NAME" \
    ./build-fic-ubuntu2404-deb.sh "$FIC_PRODUCT_VERSION"
