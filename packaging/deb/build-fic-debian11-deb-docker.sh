...
IMAGE_NAME="${IMAGE_NAME:-fic-deb-builder:debian11}"
DOCKERFILE_PATH="${DOCKERFILE_PATH:-$ROOT_DIR/packaging/deb/Dockerfile.debian11}"
...
"$CONTAINER_CMD" run --rm \
    ...
    -e PACKAGE_DISTRO_TAG="${PACKAGE_DISTRO_TAG:-debian11}" \
    -e BUILD_ROOT="${BUILD_ROOT:-/tmp/fic-build-debian11}" \
    -e DIST_DIR="${DIST_DIR:-/workspace/dist}" \
    -v "$ROOT_DIR:/workspace" \
    -w /workspace/packaging/deb \
    "$IMAGE_NAME" \
    ./build-fic-debian11-deb.sh "$PACKAGE_VERSION"
