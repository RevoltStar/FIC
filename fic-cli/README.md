...
`fic-cli` - терминальный клиент FIC. Компонент предназначен для управления конфигурацией и немедленного применения политик через демон `fic`.
...
Краткая версия продукта и полный идентификатор сборки доступны без соединения
с демоном:
...
Показывает логическую ревизию данных дерева устройств:

```bash
fic-cli device revision
```

Команда обращается к отдельному device endpoint и отправляет
`device_tree_revision`. Ревизия увеличивается при изменении устройств,
атрибутов или событий и используется GUI для условного обновления дерева.
