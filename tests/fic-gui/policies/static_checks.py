...
def main():
    ...
    policy_service = (root / "fic-gui/src/features/policies/services/PolicyService.cpp").read_text(encoding="utf-8")
    device_page = (root / "fic-gui/src/features/devices/pages/DeviceModulePage.cpp").read_text(encoding="utf-8")
    audit_page = (root / "fic-gui/src/features/policies/pages/AuditModulePage.cpp").read_text(encoding="utf-8")
    log_viewer = (root / "fic-gui/src/features/logs/widgets/LogViewer.cpp").read_text(encoding="utf-8")
    log_model = (root / "fic-gui/src/features/logs/models/LogModel.cpp").read_text(encoding="utf-8")
    global_config = (root / "fic/src/resources/config/GLOBAL.conf").read_text(encoding="utf-8")
    audit_config = (root / "fic/src/resources/config/AUDIT.conf").read_text(encoding="utf-8")
    ru_lang = (root / "fic/src/resources/lang/ru.lang").read_text(encoding="utf-8")
    en_lang = (root / "fic/src/resources/lang/en.lang").read_text(encoding="utf-8")
    ...
    require("features/devices/widgets/DeviceTree" not in main_window and "features/logs/widgets/LogViewer" not in main_window,
    ...
    require("setupUi();" in log_viewer and "initializeUI" not in log_viewer,
            "features/logs/widgets/LogViewer must own its UI")
    for source, name in [(device_page, "DeviceModulePage"),
                         (log_viewer, "features/logs/widgets/LogViewer"),
                         (log_model, "features/logs/models/LogModel")]:
