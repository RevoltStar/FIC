...
def main():
    ...
    config = (root / "fic/src/scripts/config/FIREWALL.conf").read_text()
    for policy in (
    ...
    registry = (root / "fic/src/core/main_function.cpp").read_text()
    for policy_class in (
    ...
    process_options = (
        root / "fic-common/fic-core/include/fic/core/ProcessExecutor.h"
    ).read_text()
    require(
